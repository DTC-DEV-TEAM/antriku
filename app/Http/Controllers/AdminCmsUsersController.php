...
class AdminCmsUsersController extends CBController {
	...
	public function cbInit() {
		...
		$this->col[] = array("label"=>"Technician ID","name"=>"technician_id");
		...
		$this->form[] = array("label"=>"Technician ID","name"=>"technician_id","type"=>"text","validation"=>'required|regex:/^[a-zA-Z0-9\s]+$/', 'required'=>false);			
		...
		// JS
		$this->script_js = "
							document.getElementById( 'form-group-technician_id' ).style.display = 'none';
							let x = document.getElementById( 'id_cms_privileges' ).setAttribute('onchange', 'get_value()');
							document.getElementById( 'technician_id' ).required = false;
							let privValueEdit = document.getElementById( 'id_cms_privileges' ).value;

							if( privValueEdit == 4 ){
								document.getElementById( 'form-group-technician_id' ).style.display = 'block';
							}

							function get_value(){
								
								let privValue = document.getElementById( 'id_cms_privileges' ).value;

								if( privValue == 4 ){
									document.getElementById( 'technician_id' ).setAttribute('required', 'required');
									document.getElementById( 'form-group-technician_id' ).style.display = 'block';
								}else{
									document.getElementById( 'form-group-technician_id' ).style.display = 'none';
								}
