	    ...
	    */
		...
		public function AddTransactionProcess(Request $request)
		{
			...
			$headerID = DB::table('returns_header')->insertGetId([
				...
				'frontliner_mc_in' 			=> $data['repair_strategy'],	
			...
			if($data['warranty_status'] == "OUT OF WARRANTY" && $data['downpayment'] == "YES"){
				$dp_status = 'PAID';
			}
		...
		public function EditTransactionProcess(Request $request)
		{
			...
			if(!empty($transaction_details->down_payment_url) || $transaction_details->downpayment_status == 'PAID'){
