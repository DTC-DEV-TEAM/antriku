	    ...
	    */
		...
		public function changeTransactionStatus(Request $request)
		{
            ...
            if($transaction_details[0]->repair_status == 1)
			{
				...
				if($all_data['other_problem_details'] == null){
					$other_problem_details = null;
				}else{
					$other_problem_details = implode(',',$all_data['other_problem_details']);
				}
                ...
                DB::table('returns_header')->where('id',$all_data['header_id'])->update([
					...
					'technician_mc_in'			=> $all_data['mc_in'],
					'wur_sur'					=> $all_data['wur_sur'],
					'gsx_no'					=> $all_data['gsx_no'],
					'requote'					=> $all_data['requote'],
					'quotation'					=> $all_data['quotation'],
			...
			//Save Technician 
			if($request->status_id == 'save_technician'){
				$arr = array();
				$repair_technician = DB::table('returns_header')->where('id',$request->header_id)->first()->repair_technician;
				array_push($arr, $repair_technician);
				array_push($arr, CRUDBooster::myId());
				$implode = implode(",", $arr);
				$repair_technician_id = implode(',',array_unique(explode(',', $implode)));

				if($repair_technician == null){
					DB::table('returns_header')->where('id',$request->header_id)->update([
						'repair_technician'          => CRUDBooster::myId()				
					]);
				}else{
					DB::table('returns_header')->where('id',$request->header_id)->update([
						'repair_technician'          => $repair_technician_id	
					]);					
				}
