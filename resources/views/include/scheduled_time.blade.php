	...
	$.ajax({
		...
		success : function (result) {
			...
			for(a=0; a < result.schedule.length; a++){
				...
				if(bookedTime.includes(timeString) && myTime !== timeString){
					var booked = 1;
				...
				if (date.getHours() < time && isDateToday(setDate)) {
					$('#scheduled_time').append('<option value="'+timeString+'" '+unavailable+'>'+timeString12hr+' ('+booked+'/1)</option>');
    ...
    <div class="col-md-8">
        ...
        <select name="scheduled_time" id="scheduled_time" class="form-control" required>
            @for($i=0; $i<=count($OperatingSched)-1; $i++)
                ...
                ?>
                <option value="{{ $time }}" {{ $isSelected }} {{$isDisabled}}>{{ date('h:i A', strtotime($time)) }} ({{$book_count}}/1)</option>
            @endfor
        </select>
    </div>
</div>
